...
/// @notice Minimal test base providing assertion helpers.
abstract contract TestBase {
    ...
    function callAndExpectSuccess(address target, bytes memory data, string memory message)
        internal
    ...
    function callAndExpectRevert(address target, bytes memory data, string memory message)
        internal
        returns (bytes memory)
    {
        (bool success, bytes memory result) = target.call(data);
